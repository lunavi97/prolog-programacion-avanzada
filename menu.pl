...
mayores_que_minimo(X) :- proyeccion(X), proyeccion(Y), X > Y.

%Menús con la menor cantidad de calorías
menu_minimo(E, PP, P, Cal) :- menu(E, PP, P, Cal), not(mayores_que_minimo(Cal)).


%Menús con bebida que no tienen la menor cantidad de calorías
proyeccion_con_bebida(Cal) :- menu_con_bebida(_, _, _, _, Cal).
mayores_que_minimo_con_bebida(X) :- proyeccion_con_bebida(X), proyeccion_con_bebida(Y), X > Y.

%Menús con bebida con la menor cantidad de calorías
menu_con_bebida_minimo(E, PP, P, B, Cal) :- menu_con_bebida(E, PP, P, B, Cal), not(mayores_que_minimo_con_bebida(Cal)).
